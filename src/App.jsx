...
const UI = {
  ko: {
    ...
    analyzeDisclaimer: "본 분석은 AI가 공개된 영상 메타데이터를 기반으로 수익화 구조를 자동 정리한 것이며, 해당 콘텐츠의 진위 여부나 효과를 보증하지 않습니다.",
    contactTitle: "문의하기",
    contactName: "이름",
    contactEmail: "이메일",
    contactType: "문의 유형",
  ...
  en: {
    ...
    analyzeDisclaimer: "This analysis is automatically generated by AI based on public video metadata. It does not guarantee the authenticity or effectiveness of the content.",
...
export default function App(){
  ...
  const [region, setRegion] = useState(()=>lang==='ko'?'kr':'global')
  ...
  return (
    <div className="min-h-screen">
      ...
      <main className="max-w-7xl mx-auto px-4 py-6">
        ...
        <div className="bg-gradient-to-r from-indigo-900/20 to-purple-900/20 border border-indigo-500/20 rounded-xl p-4 mb-6">
          <h2 className="text-white font-medium text-sm mb-3">{t.analyzeTitle}</h2>
          ...
          {analyzeResult&&(
            <div className="mt-4 bg-slate-900/50 border border-slate-700 rounded-lg p-4 space-y-3">
              <h3 className="text-white font-medium">{lang==='ko'?(analyzeResult.title_ko||analyzeResult.title):(analyzeResult.title_en||analyzeResult.title)}</h3>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3 text-sm">
                <div><span className="text-indigo-400 text-xs">{t.method}</span><p className="text-white">{lang==='ko'?analyzeResult.method:analyzeResult.method_en}</p></div>
                <div><span className="text-indigo-400 text-xs">{t.principle}</span><p className="text-white">{lang==='ko'?analyzeResult.principle:analyzeResult.principle_en}</p></div>
              </div>
              <div><span className="text-indigo-400 text-xs">{t.requirements}</span><p className="text-white text-sm">{lang==='ko'?analyzeResult.requirements:analyzeResult.requirements_en}</p></div>
              <div className="flex flex-wrap gap-6">
              ...
              <p className="text-slate-500 text-[10px] mt-2">{t.analyzeDisclaimer}</p>
            </div>
          )}
        </div>
        ...
        <div className="flex flex-col lg:flex-row gap-6 items-start">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="bg-slate-800/30 border border-slate-700 rounded-lg p-3 mb-4"><div className="flex items-center justify-between mb-2"><h4 className="text-slate-300 text-xs font-medium">{t.filterTitle}</h4>{hasFilters&&(<button onClick={()=>{setFilterDifficulty(null);setFilterCost(null);setFilterTime(null)}} className="text-indigo-400 text-xs hover:text-indigo-300">{t.filterReset}</button>)}</div><div className="space-y-2"><ClickableMeter label={t.difficulty} value={filterDifficulty} onChange={setFilterDifficulty} color="bg-orange-500"/><ClickableMeter label={t.cost} value={filterCost} onChange={setFilterCost} color="bg-red-500"/><ClickableMeter label={t.time} value={filterTime} onChange={setFilterTime} color="bg-yellow-500"/></div></div>
