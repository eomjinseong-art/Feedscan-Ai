...
KR_KEYWORDS = [
    # AI + 돈
    "AI 부업", "ChatGPT 돈버는법", "AI 수익화", "AI 재택",
    "AI 온라인 알바", "인공지능 부업", "AI 자동화 수익",
    # 재테크
    "주식", "재테크", "부동산",
    "배당주 추천", "코인 수익", "월급 외 수입",
    # 제휴/커머스
    "쿠팡파트너스", "네이버 블로그 수익", "스마트스토어 수익",
    # 기타
    "N잡", "온라인 알바", "부업 추천", "AI 투잡"
]
EN_KEYWORDS = [
...
def main():
    ...
    # === 한국 수집 (필터 없이 전체 → 한글 제목만 필터 → 길이로 분류) ===
    print("=== KR (ALL) ===")
    kr_all = {}
    for kw in KR_KEYWORDS[:10]:  # 10개 키워드 사용
        ...
        for vid, info in get_details(yt, ids).items():
            if info["views"] >= MIN_VIEWS and vid not in kr_all:
                # 한글 제목 필터: 제목에 한글이 1글자라도 있어야 함
                if has_korean(info["title"]):
                    info["language"] = "ko"
                    kr_all[vid] = info
