...
def get_details(yt, ids):
    ...
    for i in range(0, len(ids), 50):
        ...
        for item in r.get("items", []):
            ...
            is_short = parse_duration_seconds(duration) <= 180  # 3분 이하를 쇼츠로 분류
            details[item["id"]] = {
                "video_id": item["id"],
...
def main():
    ...
    print("\n=== US VIDEOS ===")
    video_keywords = EN_KEYWORDS[:10] + ["AI business tutorial", "AI income full guide", "AI side hustle tutorial", "how to make money AI step by step"]
    for kw in video_keywords:
        items = search_videos(yt, kw, "en", after, "long", region_code="US")
        ids = [i["id"]["videoId"] for i in items if "videoId" in i.get("id", {})]
        for vid, info in get_details(yt, ids).items():
            if info["views"] >= MIN_VIEWS and vid not in en_videos:
                if not has_non_english(info["title"]):
                    info["language"] = "en"; info["is_short"] = False; en_videos[vid] = info
    # medium 도 검색
    for kw in EN_KEYWORDS[:6]:
    ...
    if OPENAI_API_KEY:
        ...
        analyze_list(client, aitool_shorts_top10, "AI Tool Shorts")
        analyze_list(client, aitool_videos_top10, "AI Tool Videos")
    else:
        print("OPENAI_API_KEY not set, skipping analysis")
        for lst in [kr_shorts_top10, kr_videos_top10, shorts_top10, videos_top10, aitool_shorts_top10, aitool_videos_top10]:
