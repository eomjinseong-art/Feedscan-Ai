...
# 한국 쇼츠 전용 키워드 (짧은 영상이 많이 나오는 키워드)
KR_SHORTS_KEYWORDS = [
    "AI 부업 쇼츠", "AI 돈벌기 shorts", "ChatGPT 꿀팁", "AI 수익 인증",
    "쿠팡파트너스 수익", "AI 부업 후기", "AI 자동화", "GPT 돈버는법",
    "AI 쇼츠 수익", "AI 부업 시작", "제휴마케팅 수익", "AI 월수입"
]
...
# 글로벌 영상(롱폼) 전용 키워드
EN_VIDEO_KEYWORDS = [
    "AI business tutorial 2026", "how to make money with AI full guide",
    "AI side hustle step by step", "AI income strategy explained",
    "AI automation business complete", "ChatGPT money making tutorial",
    "AI freelancing full course", "passive income AI 2026",
    "AI agency business model", "build AI business from scratch"
]
...
def search_videos(yt, kw, lang, after, duration_filter=None, region_code=None):
    """duration_filter: 'short'/'medium'/'long'/None"""
    try:
...
def main():
    ...
    # ============================================================
    # 1. 한국 쇼츠 수집 (videoDuration=short 사용 + 전용 키워드)
    # ============================================================
    print("=== KR SHORTS (videoDuration=short) ===")
    kr_short_kws = KR_SHORTS_KEYWORDS + KR_KEYWORDS[:10]
    for kw in kr_short_kws:
        ...
        for vid, info in get_details(yt, ids).items():
            if info["views"] >= MIN_VIEWS and has_korean(info["title"]):
                info["language"] = "ko"
                info["is_short"] = True
                if vid not in kr_shorts:
                    kr_shorts[vid] = info
    # 추가: duration 없이 검색 후 3분 이하 필터
    for kw in KR_KEYWORDS[:8]:
        items = search_videos(yt, kw, "ko", after, None, region_code="KR")
        ids = [i["id"]["videoId"] for i in items if "videoId" in i.get("id", {})]
        for vid, info in get_details(yt, ids).items():
            if info["views"] >= MIN_VIEWS and has_korean(info["title"]) and info["is_short"]:
                info["language"] = "ko"
                if vid not in kr_shorts:
                    kr_shorts[vid] = info
    print(f"  KR shorts found: {len(kr_shorts)}")

    # ============================================================
    # 2. 한국 영상 수집 (medium + long)
    # ============================================================
    print("\n=== KR VIDEOS ===")
    for kw in KR_KEYWORDS[:14]:
        ...
        for vid, info in get_details(yt, ids).items():
            if info["views"] >= MIN_VIEWS and has_korean(info["title"]):
                info["language"] = "ko"
                info["is_short"] = False
                if vid not in kr_videos:
                    kr_videos[vid] = info
    for kw in KR_KEYWORDS[:8]:
        items = search_videos(yt, kw, "ko", after, "long", region_code="KR")
        ids = [i["id"]["videoId"] for i in items if "videoId" in i.get("id", {})]
        for vid, info in get_details(yt, ids).items():
            if info["views"] >= MIN_VIEWS and has_korean(info["title"]):
                info["language"] = "ko"
                ...
                if vid not in kr_videos:
                    kr_videos[vid] = info
    print(f"  KR videos found: {len(kr_videos)}")

    # ============================================================
    # 3. 글로벌 쇼츠 수집 (US, videoDuration=short)
    # ============================================================
    print("\n=== US SHORTS ===")
    for kw in EN_KEYWORDS[:12]:
        ...
        for vid, info in get_details(yt, ids).items():
            if info["views"] >= MIN_VIEWS and not has_non_english(info["title"]):
                info["language"] = "en"
                info["is_short"] = True
                if vid not in en_shorts:
                    en_shorts[vid] = info
    print(f"  US shorts found: {len(en_shorts)}")

    # ============================================================
    # 4. 글로벌 영상 수집 (US, medium + long + 전용 키워드)
    # ============================================================
    print("\n=== US VIDEOS ===")
    # medium 검색
    for kw in EN_KEYWORDS[:10] + EN_VIDEO_KEYWORDS[:5]:
        items = search_videos(yt, kw, "en", after, "medium", region_code="US")
        ids = [i["id"]["videoId"] for i in items if "videoId" in i.get("id", {})]
        for vid, info in get_details(yt, ids).items():
            if info["views"] >= MIN_VIEWS and not has_non_english(info["title"]):
                info["language"] = "en"
                info["is_short"] = False
                if vid not in en_videos:
                    en_videos[vid] = info
    # long 검색
    for kw in EN_KEYWORDS[:8] + EN_VIDEO_KEYWORDS[5:]:
        items = search_videos(yt, kw, "en", after, "long", region_code="US")
        ids = [i["id"]["videoId"] for i in items if "videoId" in i.get("id", {})]
        for vid, info in get_details(yt, ids).items():
            if info["views"] >= MIN_VIEWS and not has_non_english(info["title"]):
                info["language"] = "en"
                ...
                if vid not in en_videos:
                    en_videos[vid] = info
    # 필터 없이 검색 후 is_short=False만 (fallback)
    if len(en_videos) < 5:
        print("  [FALLBACK] Searching without duration filter...")
        for kw in EN_VIDEO_KEYWORDS:
            items = search_videos(yt, kw, "en", after, None, region_code="US")
            ids = [i["id"]["videoId"] for i in items if "videoId" in i.get("id", {})]
            for vid, info in get_details(yt, ids).items():
                if info["views"] >= MIN_VIEWS and not info["is_short"] and not has_non_english(info["title"]):
                    info["language"] = "en"
                    if vid not in en_videos:
                        en_videos[vid] = info
    print(f"  US videos found: {len(en_videos)}")
